...
If you are self-hosting, and using this template for onboarding, you will need to change the domain to your own.
:::

This document is your onboarding document to a fresh Guardian Connector instance hosted by CMI on the `guardianconnector.net` domain.
...
:::tip
You can replace the "alias" with your own name to get your own instance URL by adding `?alias=your-instance` to the URL. For example, `https://docs.guardin /reference/gc-toolkit/onboarding/?alias=river` will give you `https://river.guardianconnector.net/`.
:::
...
The different tools will be available at https://*.<ParamText paramName="alias" defaultValue="alias" />.guardianconnector.net/ where `*` can be one of the services currently available, such as:
...
For some services, we might have a predefined user/password combination that we will share with you when we get to them.
...
[GC Landing Page](/reference/gc-toolkit/gc-landing-page/) is a simple landing page that provides a quick overview of the tools available in your Guardian Connector instance. It is the first page that will be shown when you access your instance.

#### First time login

Let's access the Landing Page by going to <ParamLink template="https://{alias}.guardianconnector.net/" paramName="alias" defaultValue="alias">https://<ParamText paramName="alias" defaultValue="alias" />.guardianconnector.net/</ParamLink>
...
By default, Explorer will not have much to look at, unless we have set one up one or more views for you.
...
If we have already set up some data integrations for you, you will notice that there are a few folders already available in your File Browser view, and that more might appear without you doing it.
